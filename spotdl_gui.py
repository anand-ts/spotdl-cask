# spotdl_gui.py – multi-link with live status (rev-3)
"""
Bulk-paste GUI for **spotDL** with:
* Proper metadata layout (Title • Artist • Album columns).
* "Download all" turns into **Downloading…** while jobs run.
* Per-track status: idle → downloading (progress bar) → done / error.

No external websocket lib—just lightweight polling via `/status`.
"""
...
_HTML = r"""<!doctype html><html lang='en'><head><meta charset='utf-8'/><title>spotDL Bulk</title><style>:root{font-family:Arial,Helvetica,sans-serif;--green:#1db954;--gray:#eee}body{margin:0;height:100vh;display:flex;flex-direction:column;background:#fafafa}header{background:#fff;padding:1rem 1.5rem;display:flex;align-items:center;gap:1rem;box-shadow:0 2px 6px rgba(0,0,0,.05)}header h1{font-size:1.15rem;margin:0;flex:1}header button{padding:.6rem 1rem;border:none;border-radius:8px;color:#fff;background:var(--green);cursor:pointer;font-size:.9rem}header button[disabled]{opacity:.5;cursor:default}#zone{flex:1;overflow:auto;padding:1rem}table{width:100%;border-collapse:collapse}th,td{padding:.45rem .6rem;text-align:left}tr:nth-child(odd){background:#fff}tr:nth-child(even){background:#f3f7f3}img{width:48px;height:48px;border-radius:4px;object-fit:cover}.actions button{border:none;border-radius:6px;font-size:.8rem;padding:.35rem .7rem;margin-right:.4rem;cursor:pointer}.xbtn{background:#e65555;color:#fff}.dlbtn{background:var(--green);color:#fff}.bar{width:100%;height:6px;background:var(--gray);border-radius:3px;overflow:hidden;margin-top:4px}progress{width:100%;height:6px;appearance:none}progress::-webkit-progress-bar{background:var(--gray)}progress::-webkit-progress-value{background:var(--green)}.placeholder{border:2px dashed #bbb;border-radius:12px;padding:3rem;text-align:center;color:#666;max-width:460px;margin:4rem auto}</style></head><body><header><h1>spotDL Bulk Downloader</h1><button id='allBtn' onclick='dlAll()' disabled>Download All</button></header><div id='zone'><div id='ph' class='placeholder'>Paste Spotify / YouTube links (⌘V / Ctrl+V)<br>Separate with space, comma, or newline.</div><table id='tbl' style='display:none'><thead><tr><th></th><th>Title</th><th>Artist</th><th>Album</th><th>Status</th><th class='actions'>Actions</th></tr></thead><tbody></tbody></table></div><script>const body=document;const tblBody=document.querySelector('#tbl tbody');const ph=document.getElementById('ph');const allBtn=document.getElementById('allBtn');let rows={};body.addEventListener('paste',e=>{const txt=(e.clipboardData||window.clipboardData).getData('text');const links=txt.split(/[\s,]+/).filter(t=>t.startsWith('http'));if(!links.length)return;e.preventDefault();links.forEach(addRow);});function addRow(link){if(rows[link])return;const r=document.createElement('tr');r.innerHTML=`<td><img/></td><td>-</td><td>-</td><td>-</td><td class='status'>idle</td><td class='actions'><button class='dlbtn' onclick="dlOne('${link}')">Download</button><button class='xbtn' onclick="rmRow('${link}')">✕</button></td>`;tblBody.appendChild(r);rows[link]=r;ph.style.display='none';document.getElementById('tbl').style.display='table';allBtn.disabled=false;fetch('/meta',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({link})}).then(r=>r.json()).then(m=>{if(!rows[link])return;r.children[0].firstChild.src=m.cover||'';r.children[1].textContent=m.title||'(unknown)';r.children[2].textContent=m.artist||'';r.children[3].textContent=m.album||'';}).catch(()=>{});}function rmRow(l){if(!rows[l])return;tblBody.removeChild(rows[l]);delete rows[l];if(!Object.keys(rows).length){ph.style.display='block';document.getElementById('tbl').style.display='none';allBtn.disabled=true;}}function dlOne(link){if(!rows[link])return;rows[link].querySelector('.dlbtn').disabled=true;rows[link].querySelector('.status').innerHTML='<progress></progress>';fetch('/download',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({link})});}function dlAll(){Object.keys(rows).forEach(dlOne);allBtn.textContent='Downloading…';allBtn.disabled=true;}setInterval(()=>{const qs=Object.keys(rows);if(!qs.length)return;fetch('/status?links='+encodeURIComponent(qs.join(','))).then(r=>r.json()).then(st=>{qs.forEach(l=>{const s=st[l];if(!s||!rows[l])return;if(s==='done'){rows[l].querySelector('.status').textContent='✓';}else if(s==='error'){rows[l].querySelector('.status').textContent='error';rows[l].querySelector('.dlbtn').disabled=false;}else if(s==='downloading'){/* already has progress */} });if(Object.values(st).every(v=>v==='done'||v==='error')){allBtn.textContent='Download All';}});},2000);</script></body></html>"""
...
def dl_ep():
    ...
    if STATUS.get(link) in {"downloading", "done"}:
        return ("", 204)
    STATUS[link] = "queued"
    threading.Thread(target=_run, args=(link,), daemon=True).start()
    return ("", 204)
